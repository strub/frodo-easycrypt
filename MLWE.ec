...
op dshort_matrix = dmatrix dshort_R.

lemma dshort_matrix_ll : is_lossless dshort_matrix.
proof. apply/dmatrix_ll/dshort_R_ll. qed.

module type Adv_T = {
   proc guess(A : matrix, t : matrix, uv : matrix * matrix) : bool
}.
...
module type HAdv_T = {
   proc guess(sd : seed, t : matrix, uv : matrix * matrix) : bool
}.
...
module MLWE_H(Adv : HAdv_T) = {
  ...
  proc main(tr b : bool) : bool = {
    ...
    e <$ dshort_matrix;
    _A <- H sd;
    u0 <- if tr then _A * s + e else s * _A + e;
    u1 <$ duni_matrix;
